...
function drawDALRParams (temp, params) {
    ...
    // Top of lift label
    let FormattedToL = Math.round(params.tol * 3.28084).toLocaleString()
    svg.append('g').append('text')
        .attr('class', 'tollabel')
        .attr('x', x((params.tolTemp * 9 / 5) + 32 + 2))
        .attr('y', y(params.tol * 3.284084 / 1000 - 0.2))
        .html('ToL: ' + FormattedToL + ' ft')
};

function d3Update() {
    const userTemp = parseInt(document.getElementById('user-temp').value)
    if (userTemp > (soundingData[1].Temp_c * 9 / 5) + 32 + 5.4 && userTemp < 120) {
